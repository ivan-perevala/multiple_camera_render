"""
Утиліти для роботи з менеджером вікон.
"""
...
if __debug__:
    ...
    if TYPE_CHECKING:
        ...
        # Внутрішній пакунок
        # Бібліотеки Blender
        from bpy.types import Area, Context, Region, Space, Window
...
def iter_windows_areas_regions(
        context: Context,
        *,
        area_type: str = 'VIEW_3D',
        region_type: str = 'WINDOW'
) -> Iterator[Tuple[Window, Area, Region]]:
    """
    Ітератор регіонів в усіх ділянках всіх вікон програми необхідного типу.
    ...
    .. code-block:: python
        ...
        for window, area, region in iter_regions(bpy.context, area_type='VIEW_3D', region_type='WINDOW'):
            pass
        ...
    ...
    :type region_type: str, опційно
    :yield: Регіон.
    :rtype: Iterator[Tuple[`Window`_, `Area`_, `Region`_]]
    """
    for window in context.window_manager.windows:
        window: Window
...
def tag_redraw_all_regions(context: Context, *, area_type: str = 'VIEW_3D', region_type: str = 'WINDOW'):
    """
